# VQ-VAE + Gated PixelCNN + WaveNet Audio VQ-VAE
...
│   ├── best_vqvae_mnist.pt               MNIST 上训练的 VQ-VAE
...
│   ├── vqvae_labels_dataset.pt           Mini-ImageNet 类别标签（与索引对齐）
...
│   ├── vqvae_audio_token_dataset.pt      预编码音频 token 序列
...
│   ├── CIFAR/                            CIFAR-10 各阶段结果
│   ├── MNIST/                            MNIST 各阶段结果
│   ├── Mini_Image_recon/                 Mini-ImageNet VQ-VAE 结果
│   └── Mini_ImageNet_gene/               Mini-ImageNet PixelCNN 生成结果
│
├── Note.md                                实验日记（各数据集参数与结果记录）
...
## 5. 训练 / 推断流程（`Image_Ex/Train.py` 与 `Audio_Ex/Train.py`）
...
项目提供了 **7 个 Jupyter notebook**，覆盖三个数据集完整的训练/重建/生成工作流：
...
#### 8.1.3 `WaveNetBlock` —— WaveNet 解码器基本单元
...
#### 8.1.6 `WaveNet1DPrior` —— token 序列上的自回归先验
...
### 8.2 音频数据管线（`Audio_Ex/GetData.py`）
