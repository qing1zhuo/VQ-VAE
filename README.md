# VQ-VAE + Gated PixelCNN + Audio VQ-VAE

基于 PyTorch 复现 **VQ-VAE**（Vector Quantized Variational Autoencoder，van den Oord et al., 2017）及其离散码本上的自回归先验模型，涵盖 **图像** 与 **音频** 两个模态。

| 模态 | 编码器 | 量化器 | 解码器 / 先验 | 数据集 |
|------|--------|--------|---------------|--------|
| 🖼️ 图像 | CNN（下采样 4×） | `VectorQuantizerEMA` | Decoder (ConvTranspose2d) / **Gated PixelCNN** | CIFAR-10, Mini-ImageNet |
...
│
├── Image_Ex/                             图像分支代码
│   ├── GetData.py                        图像数据加载（CIFAR-10, MiniImageNetDataset, IndicesDataset）
...
│   └── Mini_ImageNet_gene.ipynb          PixelCNN 先验训练与类条件生成 demo
│
├── Audio_Ex/                             音频分支代码
...
│   ├── best_vqvae_cifar.pt               CIFAR-10 上训练的 VQ-VAE
│   ├── best_vqvae_MiniImageNet.pt        Mini-ImageNet 上训练的 VQ-VAE
│   ├── best_vqvae_audio.pt               VCTK 上训练的音频 VQ-VAE
│   ├── best_pixelcnn_prior.pt            Mini-ImageNet 索引图上训练的 PixelCNN 先验
│   ├── vqvae_indices_dataset.pt          预先编码好的训练集索引 (B,H,W)
│   ├── vqvae_labels_dataset.pt           与索引对齐的类别标签（图像）
│   ├── vqvae_audio_token_dataset.pt      预先编码好的音频 token 序列
│   └── vqvae_labels_dataset.pt           音频说话人标签（与图像标签同名，但内容不同）
│
├── data/
...
│   └── Mini_ImageNet_gene/
│
├── src/                                   （源文件已迁移至 Image_Ex/ 和 Audio_Ex/）
├── test.py                                打印 torch / CUDA / torchaudio 环境检查
├── test.md                                PixelCNN 笔记占位（暂无内容）
└── README.md
```
...
## 3. 图像分支核心实现（`Image_Ex/Model.py`）

### 3.1 `VectorQuantizerEMA` —— EMA 更新的向量量化层
...
### 3.2 `Encoder` / `Decoder` / `ResidualStack`
...
## 4. 训练 / 推断流程（`Image_Ex/Train.py` 与 `Audio_Ex/Train.py`）
...
### 5.4 训练 PixelCNN 先验 + 类条件生成

打开 `Image_Ex/Mini_ImageNet_gene.ipynb`：

1. **加载 VQ-VAE**：从 checkpoint 恢复模型与 cfg。
2. **编码全部训练样本**为索引图并缓存到 `.pt` 文件。
3. **训练 Gated PixelCNN**：调用 `Image_Ex.Train.train_prior(...)`。
4. **采样生成**：
   ```python
   latent = prior_Model.generate(labels, shape=(32, 32), batch_size=len(labels))
...
#### 6.1.3 `WaveNetBlock` —— WaveNet 解码器基本单元
...
#### 6.1.6 `WaveNet1DPrior` —— token 序列上的自回归先验
...
### 6.2 音频数据管线（`Audio_Ex/GetData.py`）
...
项目提供了 4 个 Jupyter notebook，覆盖完整的工作流：
...
- **Perplexity**：若远小于 K → **码本坍缩**（codebook collapse），需调 commitment_cost 或重启。
...
| 文件 | 内容 |
|---|---|
| `best_vqvae_cifar.pt` | CIFAR-10 上训练好的 VQ-VAE（含 `model_state_dict` / `config` / loss 曲线）。 |
| `best_vqvae_MiniImageNet.pt` | Mini-ImageNet 上训练好的 VQ-VAE。 |
| `best_vqvae_audio.pt` | VCTK 上训练好的音频 VQ-VAE（含 Encoder1D + VQ + WaveNetDecoder）。 |
| `best_pixelcnn_prior.pt` | Mini-ImageNet 索引图上训练的类条件 PixelCNN。 |
| `vqvae_indices_dataset.pt` | 预编码的训练集索引张量，加速 PixelCNN 训练。 |
| `vqvae_labels_dataset.pt` | 与 indices 对齐的类别标签（图像）。 |
| `vqvae_audio_token_dataset.pt` | 预编码的音频 token 序列，加速 WaveNet1DPrior 训练。 |
| `vqvae_labels_dataset.pt` | 音频说话人标签（与图像标签文件同名，但内容不同）。 |

> **注意**：图像和音频的标签文件同名 `vqvae_labels_dataset.pt`，分别位于不同上下文中使用时需留意。
